 ...
 * @since JDK1.0
 */
// 核心类 每个Java应用程序都有一个单独的运行时类的实例(运行时环境)
public class Runtime {
     ...
     * @see #halt(int)
     */
    // 核心方法 通过启动关闭程序来终止当前正在运行的Java虚拟机
    public void exit(int status) {
        SecurityManager security = System.getSecurityManager(); // 安全策略管理器
        if (security != null) {
            security.checkExit(status);
        }
        Shutdown.exit(status); // 退出虚拟机
    }
    ...
    public void addShutdownHook(Thread hook) {
        ...
        ApplicationShutdownHooks.add(hook); // 添加应用程序的关闭挂钩
    }
     ...
     * @since 1.3
     */
    // 核心方法 注销一个以前已注册的虚拟机关闭挂钩
    public boolean removeShutdownHook(Thread hook) {
        SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
            sm.checkPermission(new RuntimePermission("shutdownHooks"));
        }
        return ApplicationShutdownHooks.remove(hook); // 移除应用程序的关闭挂钩
    }
